...
export default function TestSmallClient() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (language === 'ta') {
        ...
        isSpecialCaseCorrect =
          (currentLetter === 'c' || currentLetter === 'f') &&
    ...
    <main className="h-screen bg-warm-gradient flex flex-col overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto w-full px-4 pt-3 flex-1 flex flex-col overflow-hidden">
        <div className="bg-amber-500 rounded-2xl shadow-lg p-3 mb-2 flex justify-between items-center flex-shrink-0">
          <button onClick={() => router.push(`/choose-language?section=writing&subsection=small&lang=${language}`)} className="px-4 py-2 bg-white/20 text-white rounded-xl font-semibold hover:bg-white/30 transition-colors text-sm">← Back</button>
          <h1 className="text-2xl font-bold text-white">Test Mode - Small Alphabets</h1>
